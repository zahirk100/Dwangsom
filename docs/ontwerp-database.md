# Ontwerp: database en loginopbouw

De afwegingen achter de keuzes. Het praktische draaiboek voor de livegang staat
in [livegang.md](livegang.md).

> **Stand van zaken.** De accounts, de tweestapsverificatie, de rollen en het
> klantportaal uit dit document zijn inmiddels gebouwd, maar draaien nog op de
> opslagdrivers uit `src/opslag.js` in plaats van op Postgres. Het schema uit
> paragraaf 5 en de rijbeveiliging uit paragraaf 6 staan als uitvoerbare SQL in
> `db/001-schema.sql` en `db/002-rijbeveiliging.sql`, klaar voor de stap naar
> Supabase. Die stap is nog niet gezet; zie paragraaf 4 voor wanneer dat loont.
